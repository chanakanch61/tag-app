...
export async function POST(req) {
  ...
  if (!templateFile || !(templateFile instanceof File)) {
    return new Response(JSON.stringify({ ok: false, message: "template required" }), { status: 400 });
  }
  if (!namesJson) {
    return new Response(JSON.stringify({ ok: false, message: "names_json required" }), { status: 400 });
  }
